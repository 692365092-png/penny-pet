...
## Review amendment: do not absorb other lifecycles into drag

_pendingHostedDockRestoreGroups -> StickyFeatureController / DockRestore lifecycle candidate. _pendingStandaloneTopologyNotes -> DisplayRuntime + Sticky feature reconcile boundary. _pendingDockTopologyGroups -> Display/Sticky topology reconcile boundary.

_dockPlanMailbox currently also allocates PlanSequence for PostDockGroupTopologyReproject and PostDockGroupRestoreReproject. Only live/final drag mailbox ownership is a DockInteractionRuntime candidate. Cross-lifecycle sequence allocation/topology/restore usage remains an independent boundary; PC-6 decides whether to split or consolidate. Hidden-group restore must not depend on an active drag runtime.
